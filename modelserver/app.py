...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if df.empty:
            logging.error("Zip_codes.csv is empty.")
        else:
...
def optimize_endpoint(request: FCRequest):
    if df.empty:
        raise HTTPException(status_code=500, detail="Server Error: Zip_codes.csv data missing.")
    try:
...
def run_model(num_customers: int, num_fc: int, k: int):
    ...
    if k > num_fc:
        raise ValueError("k cannot be greater than num_fc_candidates")
    if num_customers > len(df) or num_fc > len(df):
        raise RuntimeError(f"Not enough rows in CSV. Requested {max(num_customers, num_fc)}, available {len(df)}")
    ...
    model += (pulp.lpSum(cost_matrix[i][j] * x[i][j] for i in range(num_customers) for j in range(num_fc)) +
    ...
    try:
        ...
        if CARTOPY_AVAILABLE:
            ...
            if chosen_fc:
                ax.scatter([fc_lons[j] for j in chosen_fc], [fc_lats[j] for j in chosen_fc],
